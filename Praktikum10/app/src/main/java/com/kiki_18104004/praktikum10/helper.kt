    ...
    fun mapCursorToArrayList(notesCursor: Cursor?): ArrayList<Quote> {
        ...
        notesCursor?.apply {
            while (moveToNext()) {
                ...
                val umur =
                    getInt(getColumnIndexOrThrow(DatabaseContract.QuoteColumns.UMUR))
                val hobby =
                    getString(getColumnIndexOrThrow(DatabaseContract.QuoteColumns.HOBBY))
                val date =
                    getString(getColumnIndexOrThrow(DatabaseContract.QuoteColumns.DATE))
                quotesList.add(Quote(id, title, description,category,umur,hobby,date))
            }
        }
        return quotesList
    }
